  ...
  function createForm() {
    ...
    form.onsubmit = function(e) {
      ...
      formConfig.fields.forEach(function(field) {
        ...
        var input = document.getElementById('field-' + field.id);
        console.log('Field:', field.id, 'Label:', field.label, 'Type:', field.type, 'Input found:', !!input);
          ...
          if (field.type === 'checkbox') {
            ...
            if (input.files && input.files.length > 0) {
              ...
              for (var i = 0; i < input.files.length; i++) {
                console.log('  Adding file:', input.files[i].name, 'Size:', input.files[i].size);
                formDataObj.append(field.id, input.files[i]);
              }
            }
          } else {
            formDataObj.append(field.id, input.value);
