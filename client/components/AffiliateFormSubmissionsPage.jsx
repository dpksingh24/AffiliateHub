...
const AffiliateFormSubmissionsPage = ({ shop }) => {
  ...
  const fetchData = async () => {
    try {
      ...
      // Fetch form details
      const formResponse = await getAffiliateFormById(formId)
      setForm(formResponse.form)
      
      // Fetch submissions
      const submissionsResponse = await getAffiliateFormSubmissions(
        formId, 
        pagination.page, 
        pagination.limit
      )
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      ...
      await deleteAffiliateFormSubmission(formId, submissionToDelete.id)
  ...
  // Create customer and send email to set password
  const handleSendEmailAndCreateCustomer = async () => {
    ...
    try {
      ...
      // Create customer and send account activation email
      const result = await createCustomer(email, firstName, lastName, '')
      
      if (result.success && result.customer) {
        setApprovalCustomer(result.customer)
        setCustomerNotFound(false)
        showToast('Customer created and email sent successfully!')
      }
    } catch (err) {
  ...
  const handleApproveConfirm = async () => {
    ...
    try {
      ...
      await approveAffiliateFormSubmission(
        formId,
      ...
      // ✅ NEW: Create affiliate profile and generate referral link
      if (approvalCustomer?.id) {
        try {
          const email = findEmailInSubmission(approvalSubmission.data)
          const firstName = findFirstNameInSubmission(approvalSubmission.data)
          const lastName = findLastNameInSubmission(approvalSubmission.data)

          // Create affiliate profile with shop parameter
          const affiliateResult = await createAffiliateAndReferralLink(
            approvalCustomer.id,
            {
              email,
              firstName,
              ...
              shop  // ✅ Add shop parameter
            }
          )

          if (affiliateResult.success && affiliateResult.affiliate?.id) {
            // The referral link is now auto-generated in backend
            showToast('✅ Affiliate profile created successfully with referral link!')
          }
        } catch (affiliateErr) {
          console.error('Error creating affiliate profile:', affiliateErr)
          showToast('Submission approved but failed to create affiliate profile', true)
        }
      }
  ...
  const handleRejectConfirm = async () => {
    ...
    try {
      ...
      await rejectAffiliateFormSubmission(formId, rejectSubmissionData.id, rejectReason, rejectAttachment || null)
  ...
  const handleExport = () => {
    // TODO: Implement export functionality for affiliate forms
  ...
  if (loading) {
    return (
      <Frame>
        <NavBar />
        <Page title="Affiliate Submissions">
          <Layout>
            <Layout.Section>
              <Card>
                <Box padding="800">
  ...
  return (
    <Frame>
      ...
      <Page
        ...
        title={`Affiliate Submissions: ${form?.name || 'Loading...'}`}
      ...
      >
          ...
          <Layout.Section>
            <Card padding="0">
              {submissions.length === 0 ? (
                <Box padding="800">
                  <EmptyState
                    heading="No affiliate submissions yet"
                    image="https://cdn.shopify.com/s/files/1/0262/4071/2726/files/emptystate-files.png"
                  >
                    <p>When affiliates submit this form, their submissions will appear here.</p>
                  </EmptyState>
                </Box>
              ) : (
                <>
        ...
        <Modal
          ...
          title="Delete affiliate submission"
          primaryAction={{
            content: 'Delete',
            destructive: true,
        ...
        >
          <Modal.Section>
            <TextContainer>
              <p>
                Are you sure you want to delete this affiliate submission? This action cannot be undone.
              </p>
            </TextContainer>
          </Modal.Section>
        </Modal>
        ...
        <Modal
          ...
          title="Affiliate Submission Details"
          limitHeight
          secondaryActions={[
            {
              content: 'Close',
                      ...
                      <Box padding="400">
                        <BlockStack gap="300">
                          ...
                          {selectedSubmission.metadata.page && (
                            <InlineStack align="space-between">
                              <Text variant="bodyMd" fontWeight="semibold">Page URL</Text>
                              <Text variant="bodyMd" truncate>
                                <a href={selectedSubmission.metadata.page} target="_blank" rel="noopener noreferrer">
                                  {selectedSubmission.metadata.page}
                                </a>
                              </Text>
                            </InlineStack>
        ...
        <Modal
          ...
          title="Approve Affiliate Submission"
          ...
          <Modal.Section>
            {approvalLoading ? (
              ...
              <BlockStack gap="400">
                ...
                <Text tone="subdued">
                  Clicking "Approve" will mark the submission as approved.
                </Text>
                <BlockStack gap="200">
              ...
              <BlockStack gap="400">
                ...
                <Text>
                  ...
                  You can send an email to create the customer account, or approve without creating.
                </Text>
                ...
                <InlineStack gap="300">
                  <Button
                    variant="primary"
                    onClick={handleSendEmailAndCreateCustomer}
                    loading={creatingCustomer}
                  >
                    Send Email & Create Customer
                  </Button>
                </InlineStack>
                <BlockStack gap="200">
                  <Text fontWeight="medium">Attach file to approval email (optional)</Text>
        ...
        <Modal
          ...
          title="Reject Affiliate Submission"
          primaryAction={{
            content: 'Reject',
            destructive: true,
        ...
        >
          <Modal.Section>
            <BlockStack gap="400">
              <Text>Are you sure you want to reject this affiliate submission?</Text>
...
export default AffiliateFormSubmissionsPage