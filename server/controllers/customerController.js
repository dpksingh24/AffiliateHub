...
/** Map GraphQL Order.displayFulfillmentStatus enum to dashboard status (matches affiliate-dashboard.js). */
...
/**
 * GET /api/customers/wishlist-products?shop=...&ids=1,2,3&handles=foo,bar
 * Returns product details (id, title, handle, image, price, url) for wishlist display.
 * Used when theme wishlist only stores ids/handles; enriches with image and price from Shopify.
 */
const getWishlistProducts = async (req, res, db) => {
  try {
    const shop = req.query.shop;
    ...
    if (idsParam && String(idsParam).trim()) {
      ...
      if (ids.length > 0) {
        const url = `https://${shop}/admin/api/${SHOPIFY_API_VERSION}/products.json?ids=${ids.join(',')}&limit=50`;
    ...
    if (handlesParam && String(handlesParam).trim()) {
      const handles = String(handlesParam).split(',').map((s) => s.trim()).filter(Boolean).slice(0, 50);
      const graphqlUrl = `https://${shop}/admin/api/${SHOPIFY_API_VERSION}/graphql.json`;
      for (const handle of handles) {
        if (byId[handle]) continue;
        const query = `
        ...
        `;
        const gid = (id) => (id && id.startsWith('gid://')) ? id.replace(/.*\/(\d+)$/, '$1') : id;
        ...
        if (resp.ok) {
          ...
          if (product) {
            const id = gid(product.id);
            const variantNode = product.variants?.edges?.[0]?.node;
            const variantId = variantNode ? gid(variantNode.id) : null;
            const price = variantNode?.price || null;
    ...
    const idOrder = [];
    if (idsParam && String(idsParam).trim()) idOrder.push(...String(idsParam).split(',').map((s) => s.trim()).filter(Boolean));
    if (handlesParam && String(handlesParam).trim()) idOrder.push(...String(handlesParam).split(',').map((s) => s.trim()).filter(Boolean));
    const seen = new Set();
    ...
    res.status(500).json({ success: false, error: 'Failed to fetch wishlist products', message: error.message });
  }
};
...
/**
 * GET /api/customers/wishlist?shop=...&customerId=...
 * Returns the customer's wishlist from DB, enriched with product details. Syncs across devices.
 */
const getCustomerWishlist = async (req, res, db) => {
  try {
    const shop = req.query.shop;
    const customerId = req.query.customerId;
    ...
    const doc = await db.collection(WISHLIST_COLLECTION).findOne({ shop, customerId });
    const productIds = (doc && doc.productIds && Array.isArray(doc.productIds)) ? doc.productIds : [];
    ...
    const ids = productIds.slice(0, 50).map((id) => String(id).replace(/\D/g, '')).filter(Boolean);
    if (ids.length === 0) {
      return res.json({ success: true, items: [] });
    }
    ...
    if (resp.ok) {
      ...
      const list = data.products || [];
      const orderById = {};
      productIds.forEach((id, i) => { orderById[String(id)] = i; });
      for (const p of list) {
        const src = (p.image && p.image.src) || (p.images && p.images[0] && p.images[0].src);
        const variant = p.variants && p.variants[0] ? p.variants[0] : null;
        items.push({
      ...
      items.sort((a, b) => (orderById[String(a.id)] ?? 999) - (orderById[String(b.id)] ?? 999));
    }

    res.json({ success: true, items });
  } catch (error) {
    console.error('getCustomerWishlist error:', error);
    res.status(500).json({ success: false, error: 'Failed to load wishlist', message: error.message });
  }
};

/**
 * POST /api/customers/wishlist
 * Body: { shop, customerId, productId }
 * Adds a product to the customer's wishlist (server-side, syncs across devices).
 */
const addToCustomerWishlist = async (req, res, db) => {
  try {
    const shop = req.body?.shop || req.query.shop;
    const customerId = req.body?.customerId || req.query.customerId;
    const productId = req.body?.productId || req.query.productId;

    if (!shop || !customerId || productId == null || productId === '') {
      return res.status(400).json({ success: false, error: 'shop, customerId and productId are required' });
    }

    const id = String(productId).replace(/^gid:\/\/shopify\/Product\/|\D/g, '') || String(productId);

    await db.collection(WISHLIST_COLLECTION).updateOne(
      { shop, customerId },
      {
        $setOnInsert: { shop, customerId, createdAt: new Date() },
    ...
    console.error('addToCustomerWishlist error:', error);
    res.status(500).json({ success: false, error: 'Failed to add to wishlist', message: error.message });
  }
};

/**
 * DELETE /api/customers/wishlist?shop=...&customerId=...&productId=...
 * Removes a product from the customer's wishlist (server-side).
 */
const removeFromCustomerWishlist = async (req, res, db) => {
  try {
    const shop = req.query.shop || req.body?.shop;
    const customerId = req.query.customerId || req.body?.customerId;
    ...
    await db.collection(WISHLIST_COLLECTION).updateOne(
      { shop, customerId },
      { $pull: { productIds: id }, $set: { updatedAt: new Date() } }
    );

    res.json({ success: true, message: 'Removed from wishlist' });
  } catch (error) {
    console.error('removeFromCustomerWishlist error:', error);
    res.status(500).json({ success: false, error: 'Failed to remove from wishlist', message: error.message });
  }
};
