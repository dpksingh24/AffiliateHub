...
const createSubmission = async (req, res, db) => {
  try {
    ...
    try {
      ...
      if (submitterEmail) {
        console.log(`Checking for duplicate email: ${submitterEmail}`);
        
        // Search for existing submission with this email in the same form
        const existingSubmission = await db.collection('form_submissions').findOne({
          formId: form._id.toString(),
          $or: [
            // Check in data fields by label
            ...emailKeys.map(key => ({ [`data.${key}`]: submitterEmail })),
            // Check in data fields by field ID
            ...(form.fields || [])
        ...
        if (existingSubmission) {
          console.log(`⚠️ Duplicate submission detected for email: ${submitterEmail}`);
          return res.status(400).json({ 
            success: false, 
      ...
      // Continue with submission if duplicate check fails (non-critical error)
    }
    ...
    // Search for email in labeled data
    for (const key of emailKeys) {
      if (labeledData[key]) {
        submitterEmail = labeledData[key];
        break;
    ...
    // If email found, check for existing submission with same email for this form
    if (submitterEmail && typeof submitterEmail === 'string' && submitterEmail.trim()) {
      const emailLower = submitterEmail.toLowerCase().trim();
      
      console.log('🔍 Checking for duplicate email:', emailLower);
      
      // Get all submissions for this form
      const existingSubmissions = await db.collection('form_submissions')
        .find({ formId: form._id.toString() })
        .toArray();
      
      // Check each submission's data for matching email
      for (const existingSubmission of existingSubmissions) {
        const existingData = existingSubmission.data || {};
        
        for (const [key, value] of Object.entries(existingData)) {
          if (typeof value === 'string' && value.toLowerCase().trim() === emailLower) {
            // Check if this field is likely an email field
            const label = fieldIdToLabel[key] || key;
            const isEmailField = emailKeys.some(k => label.toLowerCase().includes(k.toLowerCase())) || value.includes('@');
            
            if (isEmailField) {
              console.log('❌ Duplicate email found in existing submission:', existingSubmission._id);
              ...
              return res.status(400).json({ 
                success: false, 
                error: 'A submission with this email address already exists for this form',
                message: 'Duplicate email detected. Each email can only submit once.'
              });
            }
          }
        }
      ...
      console.log('✅ No duplicate email found, proceeding with submission');
    }
    ...
    try {
      ...
      if (submitterEmail) {
        console.log(`📧 Sending submission received email to: ${submitterEmail}`);
        const template = await getEmailTemplate(db, form.shop, 'submission_received');
        const emailResult = await EmailService.sendSubmissionReceivedEmail(
          submitterEmail,
          form.name,
          submitterName,
      ...
      // Notify admin of new practitioner form submission
      try {
        const adminResult = await EmailService.sendNewSubmissionNotificationToAdmin(form.name, 'practitioner', applicantFullName, submitterEmail);
...
const deleteSubmission = async (req, res, db) => {
  try {
    ...
    // Remove tag from Shopify customer if this submission had one added (do not delete the customer)
    if (submission.shopifyCustomerId && submission.tagAdded) {
      await removeTagFromCustomerInShopify(db, shop, submission.shopifyCustomerId, submission.tagAdded);
    }
...
const deleteMultipleSubmissions = async (req, res, db) => {
  try {
    ...
    // Remove tag from Shopify customer for each submission that had one added (do not delete customers)
    for (const s of submissions) {
      if (s.shopifyCustomerId && s.tagAdded) {
        await removeTagFromCustomerInShopify(db, shop, s.shopifyCustomerId, s.tagAdded);
      }
    }
  ...
  try {
    ...
    const customerData = {
      customer: {
        ...
        send_email_invite: true  // THIS triggers the password setup email
      }
    };
...
/**
 * Internal helper: remove a tag from a Shopify customer. Used when deleting a submission.
 * Does not throw; logs errors so submission delete can still proceed.
 * @param {Object} db - MongoDB db
 * @param {string} shop - Shop domain
 * @param {string|number} customerId - Shopify customer ID (numeric or GID)
 * @param {string} tag - Tag to remove
 */
async function removeTagFromCustomerInShopify(db, shop, customerId, tag) {
  if (!shop || !customerId || !tag) return;
  const shopData = await db.collection('shops').findOne({ shop });
  if (!shopData || !shopData.accessToken) return;
  const numericId = String(customerId).match(/\/(\d+)$/)?.[1] || String(customerId).replace(/\D/g, '') || customerId;
  if (!numericId) return;

  try {
    const getUrl = `https://${shop}/admin/api/${process.env.SHOPIFY_API_VERSION}/customers/${numericId}.json`;
    const getResponse = await fetch(getUrl, {
      method: 'GET',
      headers: {
        'X-Shopify-Access-Token': shopData.accessToken,
    ...
    if (!getResponse.ok) {
      console.error('removeTagFromCustomerInShopify: get customer failed', numericId, await getResponse.text());
      return;
    }
    const customerData = await getResponse.json();
    const customer = customerData.customer;
    ...
    const updateUrl = `https://${shop}/admin/api/${process.env.SHOPIFY_API_VERSION}/customers/${numericId}.json`;
    const updateResponse = await fetch(updateUrl, {
      method: 'PUT',
      headers: {
        'X-Shopify-Access-Token': shopData.accessToken,
    ...
    if (!updateResponse.ok) {
      console.error('removeTagFromCustomerInShopify: update customer failed', numericId, await updateResponse.text());
    }
  } catch (err) {
    console.error('removeTagFromCustomerInShopify error:', err);
  }
}

// Remove a tag from a Shopify customer (e.g. when deleting a submission that had a tag added)
const removeTagFromCustomer = async (req, res, db) => {
  try {
    const { customerId, tag } = req.body;
    const { shop } = req.query;
    ...
    if (!getResponse.ok) {
      const errorText = await getResponse.text();
      console.error('Shopify API error (get customer for tag remove):', errorText);
      return res.status(getResponse.status).json({
        success: false,
        error: 'Failed to get customer',
        details: errorText
    ...
    if (!updateResponse.ok) {
      const errorText = await updateResponse.text();
      console.error('Shopify API error (update customer tags):', errorText);
      return res.status(updateResponse.status).json({
        success: false,
        error: 'Failed to remove tag from customer',
        details: errorText
      });
    }
    ...
    res.json({
      ...
      customer: { id: customerId, tags: newTagsStr }
    });
  } catch (error) {
...
const deleteCustomer = async (req, res, db) => {
  try {
    ...
    // Support GID (gid://shopify/Customer/123) or numeric ID
    const numericId = String(customerId).match(/\/(\d+)$/)?.[1] || String(customerId).replace(/\D/g, '') || customerId;
    if (!numericId) {
      return res.status(400).json({
        success: false,
    ...
    const deleteUrl = `https://${shop}/admin/api/${process.env.SHOPIFY_API_VERSION || '2024-01'}/customers/${numericId}.json`;
    ...
    if (!response.ok) {
      const errorText = await response.text();
      console.error('Shopify API error (delete customer):', errorText);
      return res.status(response.status).json({
        success: false,
        error: 'Failed to delete customer from store',
        details: errorText
      });
    }
    ...
    res.json({
      success: true,
      message: 'Customer deleted from store'
    });
  } catch (error) {
  ...
  try {
    ...
    try {
      ...
      if (submitterEmail) {
        ...
        const template = await getEmailTemplate(db, form.shop, 'approval');
  ...
  try {
    ...
    try {
      ...
      if (submitterEmail) {
        ...
        const template = await getEmailTemplate(db, form.shop, 'rejection');
        const attachment = req.file ? { filename: req.file.originalname, content: req.file.buffer, contentType: req.file.mimetype } : null;
